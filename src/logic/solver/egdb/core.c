...
static void initWaysTable() {
    ...
    for (int p = 1; p <= 12; p++)
        ways[0][p] = 1;
    for (int s = 0; s <= EGDB_MAX_STONES; s++)
        ways[s][1] = 1;
...
static inline uint64_t getEGDBIndex(const Board *board, int stones) {
    ...
    if (board->color == 1) {
        ...
        rel[7] = board->cells[8];
        rel[8] = board->cells[9];
        rel[9] = board->cells[10];
        rel[10] = board->cells[11];
        rel[11] = board->cells[12];
    } else {
        rel[0] = board->cells[7];
        rel[1] = board->cells[8];
        rel[2] = board->cells[9];
        rel[3] = board->cells[10];
        rel[4] = board->cells[11];
        rel[5] = board->cells[12];
        rel[6] = board->cells[0];
        rel[7] = board->cells[1];
        rel[8] = board->cells[2];
        rel[9] = board->cells[3];
        rel[10] = board->cells[4];
        rel[11] = board->cells[5];
    }
...
static int8_t crunch(int stones, uint64_t index, Board *board) {
    ...
    for (int i = LBOUND_P1; i <= HBOUND_P1; i++) {
        ...
        int next_stones = 0;
        for (int p = LBOUND_P1; p <= HBOUND_P1; p++)
            next_stones += next.cells[p];
        for (int p = LBOUND_P2; p <= HBOUND_P2; p++)
            next_stones += next.cells[p];
