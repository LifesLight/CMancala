...
static void init_game_with_config(int stones, int distribution, int moveFunc, double timeLimit, int startColor, int seedInput) {
    ...
    currentCaptureMask = 0;
    historyCount = 0;
    push_history(globalContext.board, -1, 0);
}
...
EM_JS(void, launch_gui, (), {
    ...
    const css = `
        * { box-sizing: border-box; }
        ...
        .pit { border-radius: 50%; display: flex; align-items: center; justify-content: center; background: #444; }
    ...
    document.body.insertAdjacentHTML('beforeend', "<div class='footer'><a href='https://github.com/LifesLight/CMancala' target='_blank'>CMancala v6.0</a></div>");
    window.syncHistory = function() {
        ...
        for (let t = count - 2; t >= 0; t--) {
            ...
            const mv = Module._get_history_move(t); const cp = Module._get_history_captures(t);
            for (let i = 0; i < 14; i++) {
                const el = document.getElementById("h" + t + "-" + i);
                if (el) { el.innerText = Module._get_history_stone_count(t, i); el.classList.toggle("highlight-move", i === mv); el.classList.toggle("highlight-capture", !!(cp & (1 << i))); }
            }
        }
    ...
    window.updateView = function() {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            el.classList.toggle("highlight-move", i === Module._get_last_move()); el.classList.toggle("highlight-capture", !!(Module._get_current_captures() & (1 << i)));
        }
